...
export const fetchPosts = createAsyncThunk("posts/fetchPosts", async (id) => {
  ...
  try {
    while (allPosts.length < maxPosts) {
      ...
      if (posts.length < count) { // если количество полученных постов меньше чем запрошено, то 
        
        const remaningPosts = allPosts.slice(0, maxPosts);
        const remaningProfiles = allProfiles.slice(0, maxPosts)
